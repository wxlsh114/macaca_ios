...
class AppP2b(unittest.TestCase):
    ...
    def buyYoubei(self):
        ...
        sf2='./'+now+'_048a_inv_records_macaca.png'
        driver.save_screenshot(sf2)
        sleep(2)
        driver.element_by_name('投资测试加息优贝').click()
        sleep(3)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf3='./'+now+'_048a_invest_detail_macaca.png'
        ...
        sleep(2)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf1='./'+now+'_048a_recharge_records_macaca.png'
        ...
        sleep(2)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf0='./'+now+'_048a_recharge_detail_macaca.png'
...
if __name__ == '__main__':
    ...
    now=time.strftime('%Y-%m-%d %H_%M_%S')
    filename='./'+now+'_048a_macaca_result.html'
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='51p2b of App environment iOS10.3(投资加息优贝产品(用银行卡支付:融宝支付)) test case report by Macaca',
