...
class AppP2b(unittest.TestCase):
    ...
    def buyYoubei(self):
        ...
        sf2='./'+now+'_010_inv_records_macaca.png'
        driver.save_screenshot(sf2)
        sleep(2)
        driver.element_by_name('投资优贝90 14期').click()
        sleep(3)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf3='./'+now+'_010_invest_detail_macaca.png'
        ...
        sleep(3)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf1='./'+now+'_010_usedcoupon_macaca.png'
...
if __name__ == '__main__':
    ...
    now=time.strftime('%Y-%m-%d %H_%M_%S')
    filename='./'+now+'_010_macaca_result.html'
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='51p2b of App environment iOS10.3(投资优贝(用账户余额支付+投资券抵扣)) test case report by Macaca',
