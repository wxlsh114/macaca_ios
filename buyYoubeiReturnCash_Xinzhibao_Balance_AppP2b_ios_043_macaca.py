...
class AppP2b(unittest.TestCase):
    ...
    def buyYoubei(self):
        ...
        sf2='./'+now+'_043_inv_records_macaca.png'
        driver.save_screenshot(sf2)
        sleep(2)
        driver.element_by_name('投资测试返现：现金').click()
        sleep(3)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf3='./'+now+'_043_invest_detail_macaca.png'
        ...
        sleep(2)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf4='./'+now+'_043_capital_records_macaca.png'
        driver.save_screenshot(sf4)
        sleep(2)
        driver.element_by_class_name('XCUIElementTypeCell').click()
        sleep(2)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf5='./'+now+'_043_capital_detail_macaca.png'
        ...
        sleep(2)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf6='./'+now+'_043_xinzhibao_macaca.png'
...
if __name__ == '__main__':
    ...
    now=time.strftime('%Y-%m-%d %H_%M_%S')
    filename='./'+now+'_043_macaca_result.html'
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='51p2b of App environment iOS10.3(投资返现现金优贝产品(用薪智宝+账户余额支付)) test case report by Macaca',
